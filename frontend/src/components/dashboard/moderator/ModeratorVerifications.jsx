...
import { ModeratorVerificationService } from "../../../services/api";
import {
  Table,
  TableBody,
  TableCell,
...
import VerificationDetailsDialog from "../admin/VerificationDetailsDialog";

const ModeratorVerifications = () => {
  ...
  const { data: verifications, isLoading } = useQuery({
    queryKey: ["moderator-verifications", page, status],
    queryFn: async () => {
      const response = await ModeratorVerificationService.getVerifications(page, status);
      return response;
    },
  });
  ...
  const updateVerification = useMutation({
    mutationFn: async ({ id, action, reason }) => {
      if (action === "approve") {
        return await ModeratorVerificationService.approveVerification(id);
      } else {
        return await ModeratorVerificationService.rejectVerification(id, reason);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["moderator-verifications"]);
...
export default ModeratorVerifications;