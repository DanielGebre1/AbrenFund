...
const MobileMenu = ({ isOpen, toggleMenu, onSearchClick, user }) => {
  const { logout, isLoggedIn, role } = useAuthStore(); // ⬅️ Include role
  ...
  return (
    <div className="md:hidden bg-background border-t max-h-screen overflow-y-auto">
      <div className="container mx-auto px-4 py-4 flex flex-col space-y-4">
        ...
        <Link
          to="/"
          className="py-3 text-foreground hover:text-primary font-medium"
          onClick={toggleMenu}
        >
          Home
        </Link>
        ...
        {isLoggedIn ? (
          <div className="flex flex-col space-y-3 pt-3 border-t">
            ...
            {(role === "admin" || role === "moderator") && (
              <Link to="/moderator-dashboard" onClick={toggleMenu}>
                <Button variant="outline" className="w-full justify-start">
